...
namespace TmodloaderWorldExamples.WorldExamples
{
    public class SafeHouse : GenPass
    {
        ...
        private int sX = Main.spawnTileX; //get the spawn tile for x
        private int sY = Main.spawnTileY; //get the spawn tile for y
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            progress.Message = "Generating a basic house..."; //shows this message during worldgen
            int x = 0;
            for(int X = 0; X < 10; X++, x += 12)
            {
                SpawnHouse(sX + x, sY - 30);
            }
        }
